...
INPUT_TOPIC = "traffic_light_detection_output"
OUTPUT_TOPIC = "traffic_sign_detection_output"
SUBSCRIPTION_NAME = "traffic_sign_detection_input_sub"
...
def process_message(message):
    try:
        ...
        for result in results:
            boxes = result.boxes
            for box in boxes:
                if box.cls == 11:  # Traffic sign class
                    x1, y1, x2, y2 = box.xyxy[0].cpu().numpy()
                    traffic_sign_boxes.append([float(x1), float(y1), float(x2), float(y2)])
        
        print(f"Found {len(traffic_sign_boxes)} traffic signs")
        ...
        output_data = {
            ...
            'Traffic_Signs': traffic_sign_boxes
        }
